...
%%
function [Couplings] = retrieve_energy_couplings_from_file(n_sol,m_sol,Nen,coupling_file_name)
...
Couplings.n_en = couplingObject.n_en;
Couplings.m_en = couplingObject.m_en;
...
% Retrieve the highest solution mode
max_sol_index = max(sol_indexes);
...
Couplings.EC = couplingObject.EC(1:max_sol_index,1:max_sol_index,Nen_indexes,:,:);

% Trim further
Couplings.EC = Couplings.EC(sol_indexes,sol_indexes,:,:,:);
...
Couplings.EC = Couplings.EC(:,:,non_zero_index,:,:);
