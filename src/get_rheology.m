...
% Interior_Model: Structure containing the interior model information
        ...
        % Lateral variations can be provided in three different formats
            ...
            % a spherical harmonics expansion. The maps can be generated by generated SPH_LatLon in SPH_Tools and for efficint transformation should
            % be evaluated in the lat lon grid specified there, which depends on the maximum degree of the expansion
                    % mu_latlon
                    % eta_latlon
                    % k_latlon
            % each field has the components 
                    %.lon: longitude
                    %.lat: latityde
                    %.z:  pxq grid where p stands for latitude and q for
                    %.lmax
            % for both (1) and (2) the expansion in spherical harmonics should result in a real field (there should be both m>0 and m<0 components)
...
l_max=30; % Degree determining the spatial resolution of the rheology map in SPH_LatLon, higher is better but slower
...
for ilayer=1:Numerics.Nlayers
    if ilayer==1 %layer 1
        ...
        Interior_Model(ilayer).gs0 = Interior_Model(end).Gg0*M0/(Interior_Model(ilayer).R0^2)*1e3; %1e3 bc radius is in km
        Interior_Model(ilayer).gs = Interior_Model(end).Gg*M;
    else
        %non-dimensionalise 
        Interior_Model(ilayer).R = Interior_Model(ilayer).R0/Interior_Model(end).R0;
        Interior_Model(ilayer).rho = Interior_Model(ilayer).rho0/Interior_Model(end).rho0;
        ...
        if  (~isempty(Interior_Model(ilayer).mu_variable) && max(abs(Interior_Model(ilayer).mu_variable(:,3)))>0) || (~isempty(Interior_Model(ilayer).eta_variable) && max(abs(Interior_Model(ilayer).eta_variable(:,3)))>0) || (~isempty(Interior_Model(ilayer).K_variable) && max(abs(Interior_Model(ilayer).K_variable(:,3)))>0)
        % lateral variations provided in complex spherical harmonics 
        % check that all fields are provided, identify degrees and orders of all lateral variations, make sure the resulting fields are real
        ...
        elseif (~isempty(Interior_Model(ilayer).mu_variable_p2p) && max(abs(Interior_Model(ilayer).mu_variable_p2p(:,3)))>0) || (~isempty(Interior_Model(ilayer).eta_variable_p2p) && max(abs(Interior_Model(ilayer).eta_variable_p2p(:,3)))>0) || (~isempty(Interior_Model(ilayer).K_variable_p2p) && max(abs(Interior_Model(ilayer).K_variable_p2p(:,3)))>0)
        % lateral variations provided in peak to peak variations -----------  
        % check that all fields are provided, identify degrees and orders of all lateral variations, write lateral variations in terms of complex SH
            Interior_Model(ilayer).uniform=0;
            nR=[];
            mR=[]; 
            ...
            for i=1:length(nR)
                indP_mu=find(nR(i)==Interior_Model(ilayer).mu_variable_p2p(:,1) & mR(i)==Interior_Model(ilayer).mu_variable_p2p(:,2));
                indP_eta=find(nR(i)==Interior_Model(ilayer).eta_variable_p2p(:,1) & mR(i)==Interior_Model(ilayer).eta_variable_p2p(:,2));
                indP_K=find(nR(i)==Interior_Model(ilayer).K_variable_p2p(:,1) & mR(i)==Interior_Model(ilayer).K_variable_p2p(:,2));
                if isempty(indP_mu)
            ...
            for i=1:length(nR)
                ...
                if mR_fixed==0
                    ...
                    Interior_Model(ilayer).eta_variable(end,3)=Interior_Model(ilayer).eta_p2p(i,3)/100/Delta;
                    k = k+1;
                elseif mR_fixed>0
                    % shear modulus 
                    Interior_Model(ilayer).mu_variable(end+1,1)=nR_fixed;
        ...
        elseif ~isempty(Interior_Model(ilayer).mu_latlon) || ~isempty(Interior_Model(ilayer).eta_latlon) || ~isempty(Interior_Model(ilayer).K_latlon)
        ...
        if isempty(Interior_Model(ilayer).K_latlon)
            if isempty(Interior_Model(ilayer).mu_latlon)
                ...
                Interior_Model(ilayer).K_latlon.z=ones(size(Interior_Model(ilayer).mu_latlon.z))*Interior_Model(ilayer).K;
                Interior_Model(ilayer).mu_latlon=Interior_Model(ilayer).eta_latlon; 
                Interior_Model(ilayer).mu_latlon.z=ones(size(Interior_Model(ilayer).mu_latlon.z))*Interior_Model(ilayer).mu;
            else
...
for ilayer=2:Numerics.Nlayers
    if Interior_Model(ilayer).elastic==0 % viscoelastic model
        if Interior_Model(ilayer).uniform==0 % lateral variaaruibs
            % build mu
